                ...
                <div class="col-lg-12 col-md-4 col-xs-6 mb-5">
                    <div class="mb-5" id="full-img">
                        <img style='width:100%;border-radius:5px;'
                            src='{{ url('') }}/assets/images/{{ $project->image }}'>
                    </div>
                    <strong>
                        <p class="text-center font-x1" id="title">{{ $project->title }}</p>
                    </strong>
                    <p class="text-center" id="desc">{{ $project->description }}</p>
                    <p class="text-center" id="location">{{ $project->location }}</p>
                    <p class="text-center" id="year">{{ date('Y') }}</p>
                </div>
                @foreach (\DB::table('pictures')->where('project_id', $project->id)->get() as $item)
                    <div class="col-lg-3 col-md-4 col-xs-6 thumb mb-4 retail">
                        <a class="thumbnail" href="javascript:;" data-image-id="" data-toggle="modal"
                            ...
                            <img data-title="{{ $item->title }}" data-title="{{ $item->description }}" class="img"
                                src="{{ url('') }}/assets/images/{{ $item->image }}" alt="{{ $project->title }}">
                        </a>
                    </div>
                @endforeach
...
@section('js')
    <script>
        ...
        $('.img').on('click', function() {
            ...
            // captionText.innerHTML = $(this).data('description');
        })
